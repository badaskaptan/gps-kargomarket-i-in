-- GPS Backend için Supabase tabloları (Master Database - Clean Installation)
-- Versiyon: 2.1 - Hata giderimi yapılmış clean install versiyonu
...
DROP TRIGGER IF EXISTS on_new_task_assign ON public.gorevler CASCADE;
DROP TRIGGER IF EXISTS on_gps_update ON public.gps_kayitlari CASCADE;
DROP TRIGGER IF EXISTS on_task_status_change ON public.gorevler CASCADE;

DROP FUNCTION IF EXISTS public.handle_new_user() CASCADE;
DROP FUNCTION IF EXISTS public.match_driver_by_tc(VARCHAR, VARCHAR) CASCADE;
DROP FUNCTION IF EXISTS public.auto_assign_driver() CASCADE;
DROP FUNCTION IF EXISTS public.authenticate_kargomarketing_api(VARCHAR) CASCADE;
DROP FUNCTION IF EXISTS public.update_gps_tracking() CASCADE;
...
-- Mevcut indexleri temizle
DROP INDEX IF EXISTS idx_gorevler_tc_kimlik;
DROP INDEX IF EXISTS idx_gorevler_sofor_id;
DROP INDEX IF EXISTS idx_gorevler_ilan_no;
DROP INDEX IF EXISTS idx_gorevler_durum;
DROP INDEX IF EXISTS idx_profiles_tc_kimlik;
DROP INDEX IF EXISTS idx_profiles_aktif;
DROP INDEX IF EXISTS idx_gps_kayitlari_gorev_id;
DROP INDEX IF EXISTS idx_gps_kayitlari_sofor_id;
DROP INDEX IF EXISTS idx_admin_logs_resolved;
...
CREATE INDEX idx_gorevler_tc_kimlik ON public.gorevler(tc_kimlik);
CREATE INDEX idx_gorevler_sofor_id ON public.gorevler(sofor_id);
CREATE INDEX idx_gorevler_ilan_no ON public.gorevler(ilan_no);
CREATE INDEX idx_gorevler_durum ON public.gorevler(durum);
CREATE INDEX idx_profiles_tc_kimlik ON public.profiles(tc_kimlik);
CREATE INDEX idx_profiles_aktif ON public.profiles(aktif);
CREATE INDEX idx_gps_kayitlari_gorev_id ON public.gps_kayitlari(gorev_id);
CREATE INDEX idx_gps_kayitlari_sofor_id ON public.gps_kayitlari(sofor_id);
CREATE INDEX idx_admin_logs_resolved ON public.admin_logs(resolved);

-- ==================================================
-- FUNCTIONS VE TRIGGERS
-- ==================================================

-- Function: Yeni kullanıcı için otomatik profil
...
CREATE FUNCTION public.match_driver_by_tc(
...
CREATE FUNCTION public.auto_assign_driver()
RETURNS TRIGGER AS $$
DECLARE
...
-- Function: Kargomarketing API authentication
CREATE FUNCTION public.authenticate_kargomarketing_api(
  api_key VARCHAR(255)
)
RETURNS BOOLEAN AS $$
BEGIN
...
CREATE FUNCTION public.update_gps_tracking()
RETURNS TRIGGER AS $$
BEGIN
...
INSERT INTO public.admin_logs (log_type, error_details)
VALUES ('system_init', json_build_object('version', '2.1', 'message', 'GPS Master Database initialized successfully'))
