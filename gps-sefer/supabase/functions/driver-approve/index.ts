...
serve(async (req: Request) => {
  ...
  try {
    const { api_key, ilan_no, sofor_id, driver_email, action } = await req.json()
    ...
    if (existingTask.sofor_id && existingTask.sofor_id !== sofor_id) {
      return new Response(JSON.stringify({
        success: false,
        error: `Bu ilan zaten başka bir şöföre atanmış (Şoför ID: ${existingTask.sofor_id})`
      }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      })
    }
    ...
    const { data: updatedTask, error: updateError } = await supabase
      ...
      .update({
        ...
        sefer_durumu: 'atandi', // Şoför atandı durumu
...
POST /functions/v1/driver-approve
{
  ...
  "ilan_no": "KRG2025001",
  "sofor_id": "123e4567-e89b-12d3-a456-426614174000",
